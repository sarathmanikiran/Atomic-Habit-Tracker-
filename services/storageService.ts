...
export const storageService = {
  ...
  updateHabitsOrder: (userId: string, orderedHabits: Habit[]) => {
    const data = localStorage.getItem(STORAGE_KEYS.HABITS);
    const habits: Habit[] = data ? JSON.parse(data) : [];
    const otherUsersHabits = habits.filter(h => h.userId !== userId);
    const updatedHabits = [...otherUsersHabits, ...orderedHabits];
