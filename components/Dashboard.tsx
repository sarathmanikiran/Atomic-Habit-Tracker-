...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const handleAddHabit = (e: React.FormEvent) => {
    ...
    const newHabit: Habit = {
      ...
      name: newHabitName,
      color: newHabitColor,
      monthlyGoal: getDaysInMonth(year, month)
    };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-auto">
           ...
           <div className="h-48 w-full">
             <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={dailyStats}>
                  ...
                  <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 10}} tickFormatter={(val) => new Date(val).getDate().toString()} interval={2} />
                  <YAxis hide domain={[0, 100]} />
                  <RechartsTooltip contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b', color: '#f8fafc', fontSize: '12px', borderRadius: '8px' }} itemStyle={{ color: '#10b981' }} formatter={(value: number) => [`${value}%`, 'Completion']} labelFormatter={(label) => new Date(label).toLocaleDateString(undefined, {weekday: 'long', month: 'short', day: 'numeric'})} />
        ...
        <div className="lg:col-span-4 bg-white dark:bg-[#0f172a] rounded-xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center relative">
           ...
           <div className="w-full h-48 relative flex items-center justify-center mt-4">
              <ResponsiveContainer width="100%" height="100%">
                 <PieChart>
                    <Pie data={overallProgressData} innerRadius={60} outerRadius={80} startAngle={90} endAngle={-270} paddingAngle={0} dataKey="value" stroke="none">
                      {overallProgressData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.fill} />)}
                    </Pie>
                 </PieChart>
              </ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
         <div className="lg:col-span-8 bg-white dark:bg-[#0f172a] rounded-xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
            <div className="flex justify-between items-center mb-6">
               <h3 className="font-bold text-sm uppercase tracking-wide text-slate-900 dark:text-slate-100">Weekly Progress</h3>
               <span className="text-[10px] font-bold text-slate-500 uppercase">Volume per week</span>
            </div>
            <div className="h-48 w-full">
               <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={weeklyStatsData}>
                     ...
                     <RechartsTooltip 
                        ...
                        contentStyle={{ backgroundColor: '#0f172a', borderColor: '#1e293b', color: '#f8fafc', fontSize: '11px', borderRadius: '8px' }}
                     />
         ...
         <div className="lg:col-span-4 bg-white dark:bg-[#0f172a] rounded-xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col">
            ...
            <div className="flex-1 space-y-4">
               {rankings.slice(0, 5).map((habit, i) => {
                  const pct = daysInMonth > 0 ? Math.round((habit.count / daysInMonth) * 100) : 0;
                  return (
                     <div key={habit.id} className="group">
                        <div className="flex justify-between items-end mb-1">
                           <span className="text-[11px] font-bold text-slate-700 dark:text-slate-300 truncate pr-2">{i+1}. {habit.name}</span>
               ...
               {rankings.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-center py-6">
                     <p className="text-xs text-slate-500 italic">No data recorded for {MONTHS[month]}.</p>
                  </div>
               )}
            </div>
         </div>
      ...
      <div className="bg-white dark:bg-[#0f172a] rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col">
         <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
               ...
               <p className="text-xs text-slate-500">Track your daily protocols. Drag habits to reorder.</p>
            </div>
            <button 
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800">
               <form onSubmit={handleAddHabit} className="flex flex-col md:flex-row gap-4 items-end">
                 <div className="flex-1 w-full">
                   ...
                   <input autoFocus type="text" value={newHabitName} onChange={e => setNewHabitName(e.target.value)} className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500" placeholder="e.g. Morning Run" />
                 </div>
                 <div>
                   <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1">Color</label>
                   <div className="flex space-x-1">
                     {HABIT_COLORS.map(c => (
                       <button key={c.value} type="button" onClick={() => setNewHabitColor(c.value)} className={`w-8 h-8 rounded-md transition-transform hover:scale-110 ${newHabitColor === c.value ? 'ring-2 ring-offset-2 ring-slate-400 dark:ring-slate-500 scale-110' : ''}`} style={{ backgroundColor: c.value }} />
                     ))}
                   </div>
                 </div>
            ...
            <div className="min-w-[1000px] p-4">
               <div className="flex mb-2">
                  <div className="w-64 flex-shrink-0 flex items-end justify-between pr-4 pb-2 border-r border-slate-200 dark:border-slate-800">
                     <span className="text-[10px] font-bold uppercase text-slate-400 pl-8">Habit Name</span>
               ...
               {habits.length === 0 ? (
                  ...
                  habits.map((habit, index) => (
                     <div 
                        ...
                        className={`flex items-center mb-1 hover:bg-slate-50 dark:hover:bg-slate-800/40 rounded-lg transition-colors py-1 group/row cursor-default ${draggedIdx === index ? 'opacity-30' : ''} ${dropTargetIdx === index && draggedIdx !== index ? 'border-t-2 border-emerald-500' : ''}`}
                     >
                        <div className="w-64 flex-shrink-0 flex items-center justify-between pr-4 border-r border-slate-200 dark:border-slate-800 h-8">
                           <div className="flex items-center space-x-3 pl-2">
                              <div className="cursor-grab active:cursor-grabbing text-slate-300 dark:text-slate-700 hover:text-slate-400 dark:hover:text-slate-500 transition-colors">
                                 <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
                                   <path fillRule="evenodd" d="M10 3a1.5 1.5 0 110 3 1.5 1.5 0 010-3zM10 8.5a1.5 1.5 0 110 3 1.5 1.5 0 010-3zm1.5 7a1.5 1.5 0 10-3 0 1.5 1.5 0 003 0z" clipRule="evenodd" />
                                 </svg>
                              </div>
                              <div className="w-2 h-8 rounded-sm" style={{ backgroundColor: habit.color }}></div>
                              <span className="font-bold text-xs text-slate-700 dark:text-slate-300 truncate max-w-[120px]">{habit.name}</span>
                           </div>
                           <div className="flex items-center space-x-3">
                              <span className="text-[10px] font-mono text-slate-500">{habit.monthlyGoal}</span>
                              <button 
                                onClick={(e) => handleDeleteHabit(e, habit.id)} 
                                className="text-slate-400 hover:text-red-500 opacity-20 group-hover/row:opacity-100 transition-all duration-200 p-1 rounded hover:bg-red-50 dark:hover:bg-red-900/20"
                                title="Delete habit"
                              >
                                 <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
